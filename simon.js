//log.01. css and js files are both succsessfully linked
...
$(".btn").click(function () {
	...
	//28. Call checkAnswer() after a user has clicked and chosen their answer, passing in the index of the last answer in the user's sequence. e.g. If the user has pressed red, green, red, yellow, the index of the last answer is 3.
	var index = userClickedPattern.length-1;
	checkAnswer(index);

});//end of $(".btn").click(function ()...'s brackets (~line 20)
...
function animatePress(currentColour) {
	...
	currentButton.toggleClass("pressed");
	setTimeout(function () {
	currentButton.toggleClass("pressed");	
	}, 100);
...
//22. Use jQuery to detect when a keyboard key has been pressed, when that happens for the first time, call nextSequence(). You'll need a way to keep track of whether if the game has started or not, so you only call nextSequence() on the first keypress.
...
function checkAnswer(currentLevel) {
	...
	if (userClickedPattern[currentLevel] === gamePattern[currentLevel]) {
		...
		if (userClickedPattern.length===gamePattern.length) {
			...
			console.log("missing more clicks. index: "+currentLevel+"/"+gamePattern.lenght);
		}
		
	} else {
	
		wrong();
		$("h1").html(level+" points. GAME-OVER. Press any key to restart.");
